  ...
  return (
    <Stack spacing={0.25}>
      <Typography variant="caption">
        Tổng: <strong>{row.reportTotal ?? 0}</strong> · Chờ:{" "}
        <strong>{(row.pendingCount ?? 0) + (row.draftCount ?? 0)}</strong>
      </Typography>
      <Typography variant="caption">
        Đã gửi: <strong>{row.submittedCount ?? 0}</strong> · Đã duyệt:{" "}
        <strong>{row.approvedCount ?? 0}</strong>
      </Typography>
      <Typography variant="caption" color="error.main">
        Quá hạn: <strong>{row.overdueCount ?? 0}</strong>
      </Typography>
    </Stack>
  );
}
...
export default function DashboardWorksTable({ mode, rows }: Props) {
  ...
  const initialSortField = mode === "REPORT" ? "assignmentCode" : "updatedAtUtc";
